...
func main() {
	...
	err = ch.Publish(
		"",      // exchange : When you want a single message to be delivered to a single queue, you can publish to the default 
		         // exchange with the routingKey of the queue name. 
		q.Name,  // routing key : When you want a single message to be delivered to a single queue, you can publish to the default 
		         // exchange with the routingKey of the queue name. 
		false,   // mandatory
		false,   // immediate
		amqp.Publishing{
			// ADD *DeliveryMode: amqp.Persistent* -> to make your messages survive the rabbitmq restart as well
		  // This will tell RabbitMQ to saved the undelivered messages on the disk
